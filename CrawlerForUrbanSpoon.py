	...
	for area in areas:
		...
		for restaurant in restaurants:
			if cnt > MAX:
				flag = True
				break
			if exist(restaurant,result)==False:
				print cnt
				print restaurant
				result.append(restaurant)
				html = crawler(restaurant)
				...
				score = soup.find("div", class_="average").get_text().encode('ascii','ignore').strip("\n \t").split("\n")
				score = "".join(score)
				#Address
				...
				result.append([cnt,restaurant,title,tel,address,description,type,imageUrl,price])
				cnt = cnt + 1
				file.write(str(cnt)+"\n"+title+"\n"+restaurant+"\n"+tel+"\n"+score+"\n"+address+"\n"+description+"\n"+type+"\n"+imageUrl+"\n"+price+"\n")
	file.close()
